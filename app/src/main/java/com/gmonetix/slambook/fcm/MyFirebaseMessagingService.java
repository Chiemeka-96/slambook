...
public class MyFirebaseMessagingService extends FirebaseMessagingService {
    ...
    private static int VIBRATION_TIME = 500; //ms
    ...
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        AppPref appPref = App.getAppPref();
        ...
        if (remoteMessage.getData().size() > 0) {
            ...
            if (!App.isChatActivityOpen()) {
                // play vibration
                if (appPref.getVibration()) {
                    ((Vibrator) getSystemService(Context.VIBRATOR_SERVICE)).vibrate(VIBRATION_TIME);
                }
                RingtoneManager.getRingtone(this, Uri.parse(appPref.getRingtone())).play();
